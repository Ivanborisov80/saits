...
def masked_mae_cal(mu,sigma,target,idx):
    """calculate Mean Absolute Error"""
    log_likelihood = -0.5 * (torch.log(2 * torch.pi * torch.ones_like(sigma)) + 2 * torch.log(sigma) + ((target[:,idx] - mu) ** 2) / (sigma ** 2)) # Multiplying by a tensor of ones with the same shape as sigma to ensure the result is a tensor. 
